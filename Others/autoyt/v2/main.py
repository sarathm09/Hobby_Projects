...
class MyApp(QDialog, Ui_DownloaderGUI):
	...
	def updateUi(self):
		...
		for stat in pstatus:
			...
			if n == 1:
				...
				self.p1d.setText("size: " + str(float(details[1]) / 1024)[0:6] +
	...
	def dwn(self):
		...
		Popen("python Downloader.py " + self.dloc.text())


	def movefile(self):
		if self.movebtn.isChecked():
			self.movebtn.setText("Moving")
		Popen("python Mover.py " + self.msrc.text() + " " + self.mdest.text())
